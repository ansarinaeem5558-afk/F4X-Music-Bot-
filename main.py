...
app_web = Flask('')
@app_web.route('/')
def home(): return "🔥 F4X Empire is Running!"
...
def download_engine(url, mode, f_id=None):
    ...
    if mode == 'mp3':
        ...
        # Note: Agar Dockerfile nahi hai toh sirf 720p chalega
...
async def start(u, c):
    await u.message.reply_text(f"🚀 **F4X Ultra Fixed & Ready!**\nNaeem bhai, link bhejien.\n\n{OWNER_TAG}")

async def handle_msg(u, c):
    q = u.message.text
    if "youtu" not in q: return
    st = await u.message.reply_text("🛰️ Analyzing Quality (Bypassing YouTube)...")
    try:
...
async def btn_click(u, c):
    ...
    st = await query.message.reply_text("⏳ Processing High Quality...")
    try:
    ...
    except Exception: await st.edit_text("⚠️ Download failed. FFmpeg missing?")
